...
public class AreaGrid {
	...
	/**
	 * Constructs the grid for the map. All areas within the grid are by default
	 * of type UnknownArea.
	 * @param maxSW - lower SW point of grid
	 * @param maxNE - upper NE point of grid
	 * @param areasAcross - number of areas from side to side of the grid
	 * @param areasDown - number of areas from top to bottom of grid.
	 */
	public AreaGrid(LatLng gridSW, LatLng gridNE, int areasAcross, 
			int areasDown){
		this.gridSW = gridSW;
		this.gridNE = gridNE;
		...
		for(int y = 0; y < areasDown; y++){
			for(int x = 0; x < areasAcross; x++){
				...
				grid[x][y] = new UnknownArea(areaSW, areaNE, x, y, x+"-"+y);
			}
		}
	}
	
	/**
	 * Adds an event area to the list of event areas. This provides quicker
	 * retrieval time for these areas.
	 * @param area
	 */
	...
	/**
	 * This sets this area of the grid to the passed in area. This is mostly
	 * to change the type of the area.
	 * @param area
	...
	/**
	 * Adds an area to the list of important areas. This provides quicker
	 * retrieval time for these areas.
	 * @param area
	 */
	...
	/**
	 * Gets the area that the point is in. Will return null if no such area
	 * exists.
	 * @param point
    ...
    /**
     * Gets an area based on it's name. Note that this is not efficient.<br/><br/> If
     * you want to get an important area quickly, consider adding it to
     * "importantAreas" using addAreaToImportantAreas() for quicker retrieval.<br/><br/>
	...
	public Area getArea(String name){
	  ...
	  /**
     * Gets an area based on it's default name (the one it was initialized with). 
     * Note that this is not efficient.<br/><br/> If
     * you want to get an important area quickly, consider adding it to
     * "importantAreas" using addAreaToImportantAreas() for quicker retrieval.<br/><br/>
     * It is also faster to get an Area based on its x-y coordinate.
     * @param name
	...
	/**
	 * Gets an area based on it's id. Note that this is not efficient.<br/><br/> If
     * you want to get an important area quickly, consider adding it to
     * "importantAreas" using addAreaToImportantAreas() for quicker retrieval.<br/><br/>
     * It is also faster to get an Area based on its x-y coordinate.
	 * @param id
	...
	/**
	 * Gets an area based on it's x-y coordinate. This is the most
	 * efficient way to get an Area.
	 * @param x
	 * @param y
	 * @return area, null if the area does not exist or x and y are not within
	 * the grid
	 */
	...
	public Area getArea(LatLng areaSW, LatLng areaNE){
		for(int y = 0; y < areasDown; y++){
			for(int x = 0; x < areasAcross; x++){
				Area area = grid[x][y];
				if(area.getAreaSW().equals(areaSW) &&
						area.getAreaNE().equals(areaNE)){
					return area; 
				}
			}
		}
